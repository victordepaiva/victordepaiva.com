---
layout: game
title: 'RhythMania'
collection: jogos
categories: current-projects
published: true
published_year: '2024'
work_start_year: '2024'
work_end_year:
games_capsule_image: /assets/images/games/rhythmania-library-capsule.png
date: '2024-01-01 12:00:00 -0300'
platform: pc
store_links:
  - label: Steam
    url: https://store.steampowered.com/app/2322070/RhythMania/
store_embeds:
  - '<iframe src="https://store.steampowered.com/widget/2322070/" frameborder="0" width="646" height="190"></iframe>'
play_embeds:
  - '<div style="padding:56.25% 0 0 0;position:relative;"><iframe src="https://www.spawnd.gg/-/games/embed/26?description=true" style="position:absolute;top:0;left:0;width:100%;height:100%;border:0;" allow="autoplay; encrypted-media; clipboard-write; clipboard-read; web-share; cross-origin-isolated" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe></div>'
lang: 'br'
translation_key: 'rhythmania'
i18n_path: '/games/rhythmania/'
permalink: '/br/games/rhythmania/'
tagline: 'Um jogo de ritmo de um botão inspirado pela franquia Rhythm Heaven.'
status: 'em desenvolvimento'
genre: 'jogo de ritmo de um botão'
my_roles:
  - 'produção executiva'
  - 'design narrativo'
  - 'coordenação de localização'
  - 'marketing'
  - 'game design'
  - 'ux'
overview: |-
  *RhythMania* está atualmente em desenvolvimento.

  A demo incorporada é disponibilizada pelo spawnd, plataforma na qual trabalho como product lead.
technical_work: >-
  Trabalho no projeto como produtor executivo, designer narrativo e coordenador
  de localização, além de contribuir com marketing, game design e UX.
---
