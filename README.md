...
4. Use structured front matter for game details: `tagline`, `release_date`, `work_start_year`, `work_end_year`, `status`, `genre`, `platform`, `my_roles`, `word_count`, `banner_image`, `overview`, `about_project`, `technical_work`, `tech_and_tools`, `trailer_url`, `screenshots`, `play_embeds`, `store_links`, `store_embeds`, `additional_links`, and `awards`
5. Only include front matter fields that have real content; omit empty fields
6. The game layout renders `work_start_year`/`work_end_year`, `status`, `release_date`, `genre`, `platform`, and `word_count` as a lowercase meta line directly below the tagline, above the role-badge rules. Released games combine status and date (`released september 1st 2020`; Brazilian `lançado em 1º de setembro de 2020`). `word_count` is followed by the localized `words` / `palavras` label, inserted before any parenthetical language note. Unreleased games still show `status` on its own. `my_roles` render as badges between the two rules
7. Keep Brazilian summary copy accented (`lançado`, `inglês`, `produção`) and write ordinals as `1º`. English dates may keep `1st`
8. The `my_roles` field should be a YAML list and renders as lowercase tag badges in the summary block, using the same badge style as `tech_and_tools`
